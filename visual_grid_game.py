...
class VisualGridHuntGame:
    ...
    def get_percept(self):
        """
        Step 1.1: Exposing the World Model for Search Agents.
        Provides the complete state space necessary for offline planning.
        """
        return {
...
class GridGameGUI:
    def __init__(self, root, width=10, height=10, num_food=12, num_opponents=0, walls=None, algo='BFS'):
        self.root = root
        self.root.title(f"IT3012 - Uninformed Search ({algo})")
        ...
        self.agent.active_algo = algo  # 'BFS', 'DFS', or 'UCS'
        ...
        self.label = tk.Label(root, text=f"Algo: {self.agent.active_algo} | Score: 0 | Steps: 0", font=("Arial", 14))
    ...
    def run_loop(self):
        ...
        def step():
            if not self.env.is_done():
                ...
                end_text = f"Collision! Game Over! Score: {self.env.score}" if self.env.collision else f"Finished! Final Score: {self.env.score}"
...
if __name__ == "__main__":
    root = tk.Tk()
    # Change algo to 'BFS', 'DFS', or 'UCS' to observe different path qualities
    app = GridGameGUI(root, width=12, height=12, num_food=10, num_opponents=0, algo='BFS')
    root.mainloop()